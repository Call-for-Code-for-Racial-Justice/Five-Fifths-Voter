  ...
  <div class="remove-member">
    ...
    <div ref="popup">
      <cv-modal
        ...
        size="xs"
      >
        <template slot="label">{{ candidate.name }}</template>
        <template slot="title">Remove this candidate</template>
        <template slot="content"
          ><p>
            The candidate &quot;{{ candidate.name }}&quot; will be deleted from the "{{
              ballotItem.office
            }}" contest. This cannot be undone
          </p>
        </template>
...
export default {
  ...
  methods: {
    ...
    async actionAfterHidden() {
      ...
      if (this.deleted) {
        await this.$store.dispatch('removeCandidateContest', {
          candidate: this.candidate,
          contest: this.ballotItem
        });
      }
    },
    actionRemove() {
