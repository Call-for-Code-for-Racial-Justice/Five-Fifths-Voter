  ...
  <div class="remove-member">
    ...
    <div ref="popup">
      <cv-modal
        ...
        @primary-click="actionRemoveTeamMember"
        :auto-hide-off="false"
        size="xs"
      >
        <template slot="label">{{ person.email }}</template>
        <template slot="title">Remove this team member</template>
        <template slot="content"
          ><p>If you remove this team member you can still invite them back to the team later</p>
        </template>
...
export default {
  ...
  methods: {
    ...
    async actionAfterHidden() {
      ...
      if (this.deleted) {
        await this.$store.dispatch('sendTeamInvite', {
          ...this.person,
          acl: 'removed',
          status: 'accepted'
        });
