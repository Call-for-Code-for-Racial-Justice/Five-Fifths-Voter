<template>
  <div>
    <div :style="styleObj" class="container" :class="`container--select-${which}`">
...
export default {
  name: 'StateIcon',
  props: { which: { type: String, default: '' } },
  data() {
    return {
      styleObj: {
        width: '512px',
  ...
  created() {
    ...
    this.showStagingLogin = localStorage.getItem('showStagingLogin') === 'true';
  },
  destroyed() {
  ...
  methods: {
    actionResize() {
      let w = Math.min(window.innerWidth, window.outerWidth);
      let h = Math.min(window.innerHeight, window.outerHeight) - 80;
      this.styleObj.width = `${w}px`;
      this.styleObj.height = `${h}px`;
    },
    onClick(val) {
      if (this.targetViewBox.intervalId) return;
      let code = val?.path[0]?.classList[0];
      if (code === 'state' && val?.path[0]?.classList?.contains('dc')) code = 'dc';
      if (code) this.$emit('change-region', code);
    },
    /**
     * Quartic easing in and out https://easings.net/
     * @param {number} t Time - Amount of time that has passed since the beginning of the animation. Usually range from 0 to 1.
     * @param {number} b Beginning value - The starting point of the animation.
     * @param {number} c Change in value - The amount of change needed to go from start to goal.
     * @param {number} d Duration - Amount of time the animation will take. Typically, 1 since `t` will range from 0 to 1.
    ...
    zoomToState() {
      let p;
      if (this.which) p = document.querySelector(`.${this.which}`);
      else p = document.querySelector(`.state`);
      if (!p) return;
      let rect = p.getBBox();
      if (this.which === 'dc') rect = { x: 775, y: 225, width: 40, height: 40 };
      this.targetViewBox = {
        startX: this.currentViewBox.x,
        goalX: rect.x,
        startY: this.currentViewBox.y,
        goalY: rect.y,
        startWidth: this.currentViewBox.width,
        goalWidth: rect.width,
        startHeight: this.currentViewBox.height,
        goalHeight: rect.height,
      };
    },
  },
  watch: {
    targetViewBox() {
      const frames = 50;
      let currentFrame = 0;
      this.targetViewBox.intervalId = setInterval(() => {
        const t = currentFrame / frames;
        this.currentViewBox.x = this.easeInOutQuart(
          t,
          this.targetViewBox.startX,
          this.targetViewBox.goalX - this.targetViewBox.startX,
          1
        );
        this.currentViewBox.y = this.easeInOutQuart(
          t,
          this.targetViewBox.startY,
          this.targetViewBox.goalY - this.targetViewBox.startY,
          1
        );
        this.currentViewBox.width = this.easeInOutQuart(
          t,
          this.targetViewBox.startWidth,
          this.targetViewBox.goalWidth - this.targetViewBox.startWidth,
          1
        );
        this.currentViewBox.height = this.easeInOutQuart(
          t,
          this.targetViewBox.startHeight,
          this.targetViewBox.goalHeight - this.targetViewBox.startHeight,
          1
        );
        if (t >= 1) {
          clearInterval(this.targetViewBox.intervalId);
          this.targetViewBox.intervalId = null;
        }
        currentFrame += 1;
      }, 1000 / 25);
    },
    which() {
      if (this.targetViewBox.intervalId) clearInterval(this.targetViewBox.intervalId);
      this.zoomToState();
    },
  },
};
